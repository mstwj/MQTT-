...
def connect_router_wifi(ssid, pwd, ui_callback):
    """
    STA连接路由器WiFi
    ui_callback(text1,text2) 界面刷新回调
    返回：(成功标志, ip地址)
    """
    ...
    if sta_wlan.isconnected():
        ip = sta_wlan.ifconfig()[0]
        ui_callback("WIFI OK", f"IP:{ip}")
        print("联网成功 IP:", ip)
        return True, ip
    else:
        sta_wlan.disconnect()
        sta_wlan.active(False)
        ui_callback("WIFI Connect Fail", "Enter AP Mode")
        return False, None
