...
class AudioRecorder:
    ...
    def record_fixed_seconds(self, filename="record.wav", record_time=5):
        ...
        try:
            ...
            print(
                f"\n🔴 麦克风已就绪！开始录音（对着说几句话，时长 {record_time} 秒）..."
            )
            ...
            with open(filename, "wb") as f:
                ...
                while total_pcm_bytes < target_bytes:
                    ...
                    if num_read > 0:
                        idx = 0
                        # 4 字节为一帧，精确提取带有效声音数据的后 2 个字节 (i+2 与 i+3)
            ...
            print(
                f"⏹️ 录音结束！用时: {time.time() - start_time:.1f}s，保存为: {filename}"
            )
...
recorder.record_fixed_seconds("record_test.wav", record_time=5)